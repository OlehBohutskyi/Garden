...
<div id = "wrapper">
    ...
    <div class="admin-block">
        <div class="admin-block_list">

            <div class="admin-block_list_setting">
                <p class="admin-block_list_setting__main-page">Головна сторінка</p>
            </div>
            <p class="admin-block_list__articles">Статті</p>
            <p class="admin-block_list__users">Користувачі</p>
        </div>
        <div class="admin-block_main_main-page

        <?php

        if ($_GET['page'] == 1) echo ('show');
        else echo ('');
        ...
        ">
            ...
            <form class="admin-block_main_main-page__form" method="post" action="php/update_block.php" id="main_form">
            ...
            ?>
                ...
                <label><?= $block[1]?></label>
                <select form="main_form" name="block[<?=$block[0]?>]">
                    <?php

                    foreach ($articles as $article){

                    ?>
                        <option  <?php if ($block[2] == $article[0]) echo ' selected';?>><?=$article[1]?></option>
                    <?php
        ...
        <div class="admin-block_main_articles
        <?php

        if ($_GET['page'] == 2) echo ('show');
        else echo ('');
        ...
        ">
            <form class="search-block" action="admin_panel.php?page=2" method="post">
                <input name = "search" type="text" placeholder="Пошук...">
                <button type="submit">Шукати</button>
            </form>

            <table class="admin-block_main_articles__table">
            ...
            if ($_COOKIE['role'] != 'admin') header("Location: index.php");
            ...
            if ($search == "")
            $articles = mysqli_query($connect, "SELECT * FROM article");
            else $articles = mysqli_query($connect, "SELECT * FROM article WHERE name LIKE '%".$search."%'");
            ...
            ?>
                    ...
                    <tr>
                        ...
                        <td><?= $userName ?></td>
                        <td><?= $article[5] ?></td>
                        <td><a href="red_article.php?id=<?=$article[0]?>">Редагувати</a></td>
                        <td><a href="php/deleteArticle.php?id=<?=$article[0]?>">Видалити</a></td>
                    </tr>
            ...
            <a href="new_article.php" class="admin-block_main_articles__create">Створити нову статтю</a>

        </div>
        <div class="admin-block_main_users

        <?php

        if ($_GET['page'] == 3) echo ('show');
        else echo ('');
            ...
            <table class="admin-block_main_articles__table">
                ...
                ?>
                    ...
                    <tr>
                        ...
                        if ($_COOKIE['user'] != $user[1]):

                        ?>
                        <td><a href = "php/set_unset_admin.php?id=<?php
                            echo $user[0];?>
                            ">Додати/видалити</td>
                        <?php
                        else:
                        ...
                        ?>
                        <td><a href="php/deleteUser.php?id=<?php
                            echo $user[0];?>
                        ">Видалити</a></td>
                        <?php
                        else:
...
<script src="script/admin_anim.js"></script>
