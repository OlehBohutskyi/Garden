...
<div id = "wrapper">
    ...
    <div class="new-article-block">
        <form action="php/updateArticle.php?id=<?= $_GET['id']?>" method="post" class="new-article-block_form">
            ...
            <input type="text" name="name" placeholder="Назва статті" value="<?= $article['name']?>">
            <input type="text" name="info" placeholder="Доп. інфо" value="<?= $article['info']?>">
            <textarea name="text" placeholder="Текст статті"><?= $article['text']?></textarea>
            <input type="text" name="media" placeholder="Посилання на медіа" value="<?= $article['media']?>">
            <label>Відео?</label>
            <input type="checkbox" name="video"

                   <?php

                   if ($article['video'] == 1) echo 'checked';
